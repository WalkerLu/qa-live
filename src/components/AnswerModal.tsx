...
const AnswerModal: React.FC<AnswerModalProps> = ({ isOpen, onClose, question, onSubmit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (answer.trim() || answerType === 'stage') {
      const finalAnswer = answerType === 'stage' ? t('questions.stageAnswerNote') : answer;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg max-w-2xl w-full p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-900">{t('modal.answerQuestion')}</h2>
          <button
            onClick={onClose}
        ...
        <div className="mb-6">
          ...
          <p className="text-xs text-gray-500 mt-2">{t('questions.askedBy', { name: question.submitterName })}</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-3">
              {t('modal.answerMethod')}
            </label>
            <div className="space-y-2">
              <label className="flex items-center">
                ...
                />
                <span className="ml-2 text-sm text-gray-700">{t('modal.writtenAnswer')}</span>
              </label>
              <label className="flex items-center">
                <input
                  type="radio"
                  ...
                  className="h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300"
                />
                <span className="ml-2 text-sm text-gray-700">{t('modal.liveAnswer')}</span>
              </label>
            </div>
          </div>
